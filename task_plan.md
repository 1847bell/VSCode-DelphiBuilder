...
### 阶段 6：配置与菜单重构
- [ ] 初始化本地 Git 仓库并提交 `0.1.4` 基线
- [ ] 确认 VS Code 右键菜单动态配置能力与配置选择方案
- [ ] 将 DCC 路径改为显式必填，移除默认项目和默认配置
- [ ] 按预配置列表提供 Build 菜单并强制使用传入配置
- [ ] 评估并精简 Rebuild 菜单
- [ ] 增加默认隐藏的 Show Build Plan 菜单设置
- [ ] 更新测试、文档、版本并完成打包验证
- **状态：** in_progress

## 关键问题
1. 真实 XE7 `.dproj` 的属性覆盖和 DCC 参数需由脱敏样本与 IDE 命令行校准。
2. 当前目录不是 Git 仓库；用户已明确要求开始，允许在此创建工程文件。
